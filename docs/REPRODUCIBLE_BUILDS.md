...
After a separately administered system builds the exact frozen commit, use
...
The complete raw executable comparison is authoritative. Rust/Cargo verbose
metadata is also hashed and compared, but a host-dependent metadata difference
does not override matching executable bytes; it must be explained and reviewed
