...
def _validate_hex(value: str, pattern: re.Pattern[str], label: str) -> str:
    ...
    if pattern.fullmatch(normalized) is None:
        raise ReproductionVerificationError(f"{label} must be lower-case hexadecimal")
    return normalized
...
def verify_reproduction(
    *,
    ...
    if any(reference.metadata[field] != reproduced.metadata[field] for field in identity_fields):
        raise ReproductionVerificationError(
            "reproduced candidate identity or toolchain metadata differs"
        )
    ...
    return {
        ...
        "comparison": {
            ...
            "identity_and_toolchain_metadata_equal": True,
